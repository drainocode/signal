...
export function CampaignStats({ companies, contacts }: CampaignStatsProps) {
  ...
  const verifiedPct = percent(verifiedCount, contacts.length);
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
      ...
      <StatCard label="Targets" value={companies.length} size="sm" />
      <StatCard label="Decision Makers" value={contacts.length} size="sm" />
      <StatCard
        label="Verified Emails"
        value={verifiedCount}
        sublabel={contacts.length > 0 ? `${verifiedPct}%` : undefined}
        size="sm"
      />
      <StatCard
        label="Contacted"
