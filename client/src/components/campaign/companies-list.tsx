    ...
    return (
      <div className="border-border flex flex-col items-center gap-3 rounded-lg border border-dashed px-6 py-12 text-center">
        <div className="space-y-1">
          <p className="text-sm font-medium">No targets in this mandate</p>
          <p className="text-muted-foreground text-xs">
            Ask the agent to find acquisition targets matching your mandate criteria.
          </p>
        </div>
        <Button
          size="sm"
          onClick={() =>
            openAgentWith(
              "Find acquisition targets that match this mandate's criteria and add the best ones to the pipeline.",
            )
          }
        >
          Find targets with the agent
        </Button>
      </div>
    );
  }
        ...
        <section className="space-y-3">
          {hasOtherRegions && (
            <h3 className="text-muted-foreground text-xs font-medium uppercase tracking-wide">
              Companies with identified owners ({companiesWithLeads.length})
            </h3>
          )}
                ...
                >
                  <div className="flex items-stretch">
                        ...
                        <div className="text-muted-foreground mt-0.5 flex items-center gap-2 text-xs">
                          ...
                          {companyContacts.length > 0 && (
                            <span>
                              ...
                              {companyContacts.length === 1 ? "decision maker" : "decision makers"}
                              {enrichedCount > 0 &&
                    ...
                    <div className="flex shrink-0 items-center gap-2 pr-4">
                      {/* Phone — shown in header row with icon */}
                      {(() => {
                        const enrichData = company.enrichment_data as Record<string, unknown> | undefined;
                        const website = enrichData?.website as Record<string, unknown> | undefined;
                        const phones = website?.phones as string[] | undefined;
                        const phone = phones?.[0];
                        return phone ? (
                          <a
                            href={`tel:${phone.replace(/\s/g, "")}`}
                            className="text-muted-foreground hover:text-foreground inline-flex items-center gap-1 text-xs tabular-nums transition-colors"
                          ...
                          >
                            <svg className="h-3 w-3 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.38 2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6.29 6.29l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
                            </svg>
                            {phone}
                          </a>
                        ) : null;
...
function CompaniesWithoutLeads({
  ...
  return (
    <section className="space-y-3">
      <h3 className="text-muted-foreground text-xs font-medium uppercase tracking-wide">
        Companies without identified owners ({companies.length})
      </h3>
      <div className="border-border overflow-hidden rounded-lg border">
        <button
          ...
          aria-label={
            isOpen
              ? "Collapse companies without identified owners"
              : "Expand companies without identified owners"
          }
          className={cn(
            ...
            {companies.map((company) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Find Owner — triggers agent to run Stage 5 contact discovery */}
                  <Button
                    size="xs"
                    variant="outline"
                    onClick={() =>
                      openAgentWith(
                        `Find the owner or founder of ${company.name}. Search the website, LinkedIn, and any available sources to identify the decision maker and their contact email.`,
                      )
                    }
                  >
...
function UnassignedContacts({
  ...
  return (
    <section className="space-y-3">
      <h3 className="text-muted-foreground text-xs font-medium uppercase tracking-wide">
        Unassigned contacts ({contacts.length})
      </h3>
      <div className="border-border overflow-hidden rounded-lg border">
        <button
          type="button"
          ...
          aria-expanded={isOpen}
          aria-label={
            isOpen ? "Collapse unassigned contacts" : "Expand unassigned contacts"
          }
          className={cn(
            "hover:bg-muted/30 flex w-full items-center gap-3 px-4 py-3 text-left transition-colors",
            ROW_FOCUS,
